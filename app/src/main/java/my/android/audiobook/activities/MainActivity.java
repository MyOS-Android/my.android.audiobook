...
import my.android.audiobook.dialogs.FileDialog;
import my.android.audiobook.helpers.Migrator;
import my.android.audiobook.listeners.PlayStatusChangeListener;
import my.android.audiobook.listeners.SynchronizationStateListener;
import my.android.audiobook.models.Album;
import my.android.audiobook.models.AudioFile;
import my.android.audiobook.receivers.PlayStatusReceiver;
import my.android.audiobook.services.MediaPlayerService;
import my.android.audiobook.R;
import my.android.audiobook.helpers.Synchronizer;
import my.android.audiobook.adapters.AlbumCursorAdapter;
import my.android.audiobook.data.BookContract;
import my.android.audiobook.utils.DBAccessUtils;
...
public class MainActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor>, PlayStatusChangeListener, SynchronizationStateListener {
    ...
    public Loader<Cursor> onCreateLoader(int i, Bundle bundle) {
        ...
        if (sortOrderPref.equals(getString(R.string.settings_sort_order_by_directory_value))) {
            sortOrder += BookContract.AlbumEntry.COLUMN_DIRECTORY + " ASC, ";
        }
        sortOrder += "CAST(" + BookContract.AlbumEntry.COLUMN_TITLE + " as SIGNED) ASC, LOWER(" + BookContract.AlbumEntry.COLUMN_TITLE + ") ASC";
        return new CursorLoader(this, BookContract.AlbumEntry.CONTENT_URI, Album.getColumns(), null, null, sortOrder);
    }
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor cursor) {
        ...
        if (mPlayer != null) {
            ...
            if (cursor != null && cursor.getCount() > 0) {
                cursor.moveToFirst();
                while (true) {
                    int id = cursor.getInt(cursor.getColumnIndex(BookContract.AlbumEntry._ID));
                    currPosition++;
                    if (id == albumId) {
