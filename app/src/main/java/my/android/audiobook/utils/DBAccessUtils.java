package my.android.audiobook.utils;
...
public class DBAccessUtils {
    ...
    public static int[] getAlbumTimes(Context context, long albumID) {
        ...
        while (c.moveToNext()) {
            sumDuration += c.getInt(c.getColumnIndex(BookContract.AudioEntry.COLUMN_TIME));
            sumCompletedTime += c.getInt(c.getColumnIndex(BookContract.AudioEntry.COLUMN_COMPLETED_TIME));
        }
        c.close();
    ...
    public static boolean deleteTrackFromDB(Context context, long trackId) {
        Uri deleteUri = ContentUris.withAppendedId(BookContract.AudioEntry.CONTENT_URI, trackId);
    ...
    public static boolean deleteAlbumFromDB(Context context, long albumId) {
        ...
        Uri uri = ContentUris.withAppendedId(BookContract.AlbumEntry.CONTENT_URI, albumId);
        String[] proj = new String[]{BookContract.AlbumEntry.COLUMN_TITLE};
        ...
        String title = null;
        if (c.moveToNext()) {
            title = c.getString(c.getColumnIndex(BookContract.AlbumEntry.COLUMN_TITLE));
        }
        c.close();
    ...
    public static void markTrackAsNotStarted(Context context, long trackId) {
        ...
        // Mark as not started
        Uri uri = ContentUris.withAppendedId(BookContract.AudioEntry.CONTENT_URI, trackId);
        ContentValues values = new ContentValues();
        values.put(BookContract.AudioEntry.COLUMN_COMPLETED_TIME, 0);
    ...
    public static void markTrackAsCompleted(Context context, long trackId) {
        ...
        String[] columns = new String[]{BookContract.AudioEntry.COLUMN_TIME};
        String sel = BookContract.AudioEntry._ID + "=?";
        ...
        int totalTime = 0;
        while (c.moveToNext()) {
            totalTime = c.getInt(c.getColumnIndex(BookContract.AudioEntry.COLUMN_TIME));
        }
        c.close();
        ...
        Uri uri = ContentUris.withAppendedId(BookContract.AudioEntry.CONTENT_URI, trackId);
        ContentValues values = new ContentValues();
        values.put(BookContract.AudioEntry.COLUMN_COMPLETED_TIME, totalTime);
        context.getContentResolver().update(uri, values, null, null);
    }
}
